...
import { ChevronLeft, Clock, Send, RotateCcw, PenTool, AlertTriangle, CheckCircle, BookOpen } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold bg-primary/10 text-primary px-2 py-1 rounded-lg">25 marks</span>
          </div>
          <p className="text-muted text-xs">{essay.year ? `AQA ${essay.year} Paper 3` : "AQA Specimen"}</p>
        </div>
      </div>
      ...
      {!submitted && (
        <div className="flex items-center gap-3">
          {!timerOn ? (
            ...
            >
              <Clock className="w-4 h-4" /> Start 45-Min Timer
            </button>
          ) : (
            <div className="flex items-center gap-3">
              <div className={`text-2xl font-mono font-bold ${timeLeft < 300 ? "text-red" : "text-primary"}`}>
                {formatTime(timeLeft)}
              </div>
              <button onClick={() => setTimerOn(false)} className="text-xs text-muted hover:text-text-secondary">
                Pause
              </button>
            </div>
          )}
      ...
      <div className="dashboard-card border-l-4 border-l-primary">
        <p className="text-text-primary font-medium leading-relaxed text-lg">{essay.title}</p>
      ...
      {!submitted ? (
        <div className="space-y-3">
          ...
          <textarea
            ...
            className="w-full h-[32rem] bg-surface border border-border rounded-xl p-5 text-text-primary placeholder:text-muted focus:border-primary focus:outline-none resize-none text-[15px]"
            placeholder="Write your essay here. Aim for 3-4 sides in the exam (about 600-900 words)..."
            style={{
              lineHeight: "2.2em",
